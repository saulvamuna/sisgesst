<div>
    ...
    <div class="grid grid-cols-4">
        <select wire:model="consequenceLevel"
            ...
            @foreach ($consequenceLevels as $consequence)
                <option value="{{ $consequence->id }}">{{ $consequence->name }}</option>
            @endforeach
        </select>
        <div class="col-span-1 ml-2 mt-2 flex items-center rounded-lg justify-center border"
            style="background-color: {{ $consequenceColor }};">
            <p class="text-xl font-semibold text-white">{{ $consequenceValue }}</p>
        </div>
    </div>
    <x-selectedMeaning>
        <span class="text-md">{{ $consequenceyMeaning }}</span>
    </x-selectedMeaning>
</div>
